...
- **WHEN** the user initiates an export or document generation while one or more product checkboxes are selected
- **THEN** the system strictly applies the operation to the selected products only
...
- **WHEN** the user has selected 3 products and clicks "Exporter CSV"
- **THEN** the application downloads a CSV file containing only the 3 selected products with their full catalog attributes
