# Client Advance Refund Specification

## Purpose
Provides a formal, auditable cash refund mechanism allowing cashiers to disburse funds back to nominative clients holding an advance balance (`current_balance < 0`), with row-level concurrency locking, strict ceiling enforcement, and printable dual-signature discharge vouchers (Bon de Décharge).
