...
router.post('/initial-receipt', authenticate, requireRole('ADMIN', 'MANAGER'), async (req: AuthRequest, res) => {
...
router.post('/adjustments', authenticate, requireRole('ADMIN', 'MANAGER', 'ACCOUNTANT'), async (req: AuthRequest, res) => {
  ...
  try {
    const result = await runTransaction(async (client) => {
      ...
      if (stock) {
        await client.query(
          'UPDATE stock SET physical_quantity = $1, updated_at = NOW() WHERE id = $2',
          [newQty, stock.id]
        );
      } else {
