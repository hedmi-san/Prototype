...
router.post('/', authenticate, requireRole('ADMIN', 'SUPER_MANAGER', 'MANAGER', 'ACCOUNTANT'), async (req: AuthRequest, res) => {
  try {
...
router.put('/:id', authenticate, requireRole('ADMIN', 'SUPER_MANAGER', 'MANAGER'), async (req: AuthRequest, res) => {
  try {
    ...
    if (!current) return sendError(res, `Salary record not found with id ${id}`, 404);
    ...
    await logAudit(
      ...
      'SALARY',
      id,
      `Updated salary of ${current.employee_name} (${updatedPeriod}) to ${updatedTotal} DZD`,
...
router.delete('/:id', authenticate, requireRole('ADMIN', 'SUPER_MANAGER', 'MANAGER'), async (req: AuthRequest, res) => {
  try {
    const id = Number(req.params.id);
    const currentRes = await query(`
      SELECT s.*, e.full_name as employee_name, w.name as warehouse_name
    ...
    await logAudit(
      ...
      'SALARY',
      id,
      `Deleted salary record of ${current.employee_name} for period ${current.period} (${current.total_amount} DZD)`,
      current.warehouse_id,
      JSON.stringify(current),
      null
    );
