...
router.get('/', authenticate, (req: AuthRequest, res) => {
  ...
  const expenses = db.prepare(query).all(...params).map((e: any) => ({
    id: e.id,
...
router.post('/', authenticate, requireRole('ADMIN', 'MANAGER', 'ACCOUNTANT'), (req: AuthRequest, res) => {
...
router.put('/:id', authenticate, requireRole('ADMIN', 'MANAGER', 'ACCOUNTANT'), (req: AuthRequest, res) => {
  ...
  const current = db.prepare('SELECT * FROM expenses WHERE id = ?').get(id) as any;
