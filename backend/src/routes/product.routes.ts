...
router.get('/', authenticate, (req: AuthRequest, res) => {
  const products = db.prepare('SELECT * FROM products ORDER BY id ASC').all().map((p: any) => ({
    id: p.id,
...
router.post('/', authenticate, requireRole('ADMIN', 'MANAGER'), (req: AuthRequest, res) => {
  ...
  const p = db.prepare('SELECT * FROM products WHERE id = ?').get(newId) as any;
...
router.put('/:id', authenticate, requireRole('ADMIN', 'MANAGER', 'ACCOUNTANT'), (req: AuthRequest, res) => {
  const id = Number(req.params.id);
