...
router.get('/', authenticate, (req: AuthRequest, res) => {
  ...
  const employees = db.prepare(query).all(...params).map((e: any) => ({
    id: e.id,
...
router.put('/:id', authenticate, requireRole('ADMIN', 'MANAGER'), (req: AuthRequest, res) => {
  const id = Number(req.params.id);
  const { fullName, nationalId, phone, position, baseSalary, active } = req.body;

  const current = db.prepare('SELECT * FROM employees WHERE id = ?').get(id) as any;
