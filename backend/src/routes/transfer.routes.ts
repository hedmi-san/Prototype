...
router.post('/bulk-relocation', authenticate, requireRole('ADMIN', 'SUPER_MANAGER'), async (req: AuthRequest, res) => {
  try {
    ...
    const result = await runTransaction(async (client) => {
      ...
      const prodsMap = new Map<number, { name: string; reference: string }>(
        prodsRes.rows.map((p: any) => [p.id, { name: p.name, reference: p.reference }])
      );
      ...
      for (const dist of distributions) {
        ...
        const destWh = destWhsRes.rows.find((w: any) => w.id === destId);
    ...
    await logAudit(
      ...
      'WAREHOUSE',
      sourceWarehouseId,
      `Relocalisation de stock depuis l'entrepôt ${sourceWh.name} (${result.transfers.length} transferts créés et approuvés pour expédition)`,
      sourceWarehouseId
    );
  ...
  try {
    ...
    await runTransaction(async (client) => {
      ...
      for (const curItem of currentItems) {
        ...
        const stockRes = await client.query(
          'SELECT * FROM stock WHERE warehouse_id = $1 AND product_id = $2 FOR UPDATE',
          [transfer.source_warehouse_id, curItem.product_id]
        );
        const stock = stockRes.rows[0];
        const physQty = stock ? Number(stock.physical_quantity) : 0;
        const resQty = stock ? Number(stock.reserved_quantity) : 0;
        ...
        await client.query(
          'UPDATE transfer_items SET approved_quantity = $1 WHERE id = $2',
          [qtyToApprove, curItem.id]
        );
      }
    ...
    await runTransaction(async (client) => {
      ...
      for (const item of itemsRes.rows) {
        ...
        const destStockRes = await client.query(
          'SELECT * FROM stock WHERE warehouse_id = $1 AND product_id = $2 FOR UPDATE',
          [transfer.destination_warehouse_id, item.product_id]
        );
        ...
        if (destStock) {
          ...
          await client.query(
            'INSERT INTO stock (warehouse_id, product_id, physical_quantity, reserved_quantity) VALUES ($1, $2, $3, 0)',
            [transfer.destination_warehouse_id, item.product_id, qty]
          );
        }
