...
router.get(['/dashboard', '/dashboard-metrics'], authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    const salesTrend = trendRes.rows.map((r: any) => ({
      date: r.period_key,
      label: r.period_key,
    ...
    `);
    const recentMovements = recentMovementsRes.rows.map((m: any) => ({
      id: m.id,
    ...
    `);
    const recentSales = recentSalesRes.rows.map((s: any) => ({
      id: s.id,
...
router.get('/stock-valuation', authenticate, async (req: AuthRequest, res) => {
  try {
    const warehouseId = req.query.warehouseId ? Number(req.query.warehouseId) : undefined;
    let currentWarehouse: any = null;
    if (warehouseId) {
...
router.get('/sales', authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    const result = await query(sql, params);
    const sales = result.rows.map((s: any) => ({
      id: s.id,
      invoiceNumber: s.invoice_number,
      warehouseId: s.warehouse_id,
      warehouseName: s.warehouse_name,
...
router.get('/financial', authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    const expRes = await query(expQuery, expParams);
    const expenseCategories = expRes.rows.map((e: any) => ({
      category: e.category,
