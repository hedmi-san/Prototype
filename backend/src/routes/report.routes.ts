...
router.get(['/dashboard', '/dashboard-metrics'], authenticate, (req: AuthRequest, res) => {
  ...
  `;
  if (warehouseId) salesTodayQuery += ` AND warehouse_id = ${warehouseId}`;
  const salesTodayStats = db.prepare(salesTodayQuery).get() as any;
  ...
  `;
  if (warehouseId) cogsQuery += ` AND s.warehouse_id = ${warehouseId}`;
  const cogsStats = db.prepare(cogsQuery).get(currentMonth) as any;
  ...
  const expStats = db.prepare(expQuery).get(currentMonth) as any;
  ...
  const salStats = db.prepare(salQuery).get(currentMonth) as any;
...
router.get('/stock-valuation', authenticate, (req: AuthRequest, res) => {
  ...
  const totalPhysicalItems = rows.reduce((acc: number, r: any) => acc + r.physicalQuantity, 0);
