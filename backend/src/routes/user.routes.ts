...
router.put('/:id', authenticate, requireRole('ADMIN'), (req: AuthRequest, res) => {
  ...
  if (roleName) {
    const roleRecord = db.prepare('SELECT id, name FROM roles WHERE name = ?').get(roleName) as any;
    if (!roleRecord) {
...
router.delete('/:id', authenticate, requireRole('ADMIN'), (req: AuthRequest, res) => {
  ...
  const salesCount = (db.prepare('SELECT COUNT(*) as cnt FROM sales WHERE user_id = ?').get(targetId) as any)?.cnt || 0;
  const transfersCount = (db.prepare('SELECT COUNT(*) as cnt FROM transfers WHERE requested_by_user_id = ?').get(targetId) as any)?.cnt || 0;
