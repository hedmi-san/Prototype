...
router.put('/:id', authenticate, requireRole('ADMIN', 'MANAGER', 'ACCOUNTANT'), async (req, res) => {
    ...
    `, [updatedWarehouseId, updatedCategory, updatedAmount, updatedDesc, updatedDate, id]);
        ...
        await logAudit(req.user, 'EXPENSE_UPDATED', 'EXPENSE', id, `Updated ${updatedCategory} expense to ${updatedAmount} DZD`, updatedWarehouseId, JSON.stringify(current), JSON.stringify(updated));
...
router.delete('/:id', authenticate, requireRole('ADMIN', 'MANAGER', 'ACCOUNTANT'), async (req, res) => {
    try {
        const id = Number(req.params.id);
        const currentRes = await query('SELECT * FROM expenses WHERE id = $1', [id]);
        const current = currentRes.rows[0];
        ...
        await logAudit(req.user, 'EXPENSE_DELETED', 'EXPENSE', id, `Deleted ${current.category} expense of ${current.amount} DZD`, current.warehouse_id, JSON.stringify(current), null);
