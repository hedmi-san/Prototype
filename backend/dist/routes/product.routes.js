...
router.post('/import-batch', authenticate, requireRole('ADMIN', 'MANAGER'), async (req, res) => {
    try {
      ...
      `;
        ...
        await logAudit(req.user, 'EXCEL_IMPORT_BATCH', 'STOCK', String(targetWarehouseId), `Imported batch of ${result.processed} items into ${warehouse.name} (${result.created} created, ${result.existing} matched existing)`);
        return sendSuccess(res, {
            ...result,
