...
router.get('/', authenticate, (req, res) => {
  ...
  `;
    ...
    LIMIT ? OFFSET ?
  `;
    const transferRows = db.prepare(selectQuery).all(...params, limit, offset);
    ...
    `;
        const itemRows = db.prepare(itemsQuery).all(...transferIds);
...
router.post('/:id/approve', authenticate, (req, res) => {
    ...
    try {
        runTransaction(() => {
            ...
            const currentItems = db.prepare('SELECT * FROM transfer_items WHERE transfer_id = ?').all(id);
            for (const curItem of currentItems) {
                ...
                const stock = db.prepare('SELECT * FROM stock WHERE warehouse_id = ? AND product_id = ?').get(transfer.source_warehouse_id, curItem.product_id);
    ...
    try {
        runTransaction(() => {
            ...
            for (const item of items) {
                ...
                const sourceStock = db.prepare('SELECT * FROM stock WHERE warehouse_id = ? AND product_id = ?').get(transfer.source_warehouse_id, item.product_id);
        ...
        `).run(transfer.source_warehouse_id, item.product_id, -qty, transfer.transfer_number, `Transfert sortant vers ${transfer.destination_warehouse_name}`);
                ...
                const destStock = db.prepare('SELECT * FROM stock WHERE warehouse_id = ? AND product_id = ?').get(transfer.destination_warehouse_id, item.product_id);
                if (destStock) {
