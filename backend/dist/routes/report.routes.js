...
router.get(['/dashboard', '/dashboard-metrics'], authenticate, (req, res) => {
  ...
  `;
    ...
    const periodSalesStats = db.prepare(periodSalesQuery).get(...periodSalesParams);
  ...
  `;
    ...
    const priorSalesStats = db.prepare(priorSalesQuery).get(...priorSalesParams);
    ...
    const salesTodayStats = db.prepare(salesTodayQuery).get(...salesTodayParams);
  ...
  `;
    ...
    const cogsStats = db.prepare(cogsQuery).get(...cogsParams);
    ...
    const expStats = db.prepare(expQuery).get(...expParams);
    ...
    const salStats = db.prepare(salQuery).get(...salParams);
    ...
    const trendRows = db.prepare(trendQuery).all(...trendParams);
  ...
  `;
    ...
    const outOfStockCount = db.prepare(outOfStockQuery).get()?.count || 0;
    let lowStockQuery = `
    SELECT COUNT(*) as count
    FROM stock s
    JOIN products p ON s.product_id = p.id
  ...
  `;
    ...
    `).get(wh.id, startDate, endDate)?.sales || 0;
...
router.get('/stock-valuation', authenticate, (req, res) => {
    const warehouseId = req.query.warehouseId ? Number(req.query.warehouseId) : undefined;
    const currentWarehouse = warehouseId ? db.prepare('SELECT name FROM warehouses WHERE id = ?').get(warehouseId) : null;
    let query = `
