...
router.put('/:id', authenticate, requireRole('ADMIN'), async (req, res) => {
    try {
      ...
      `, [id]);
            ...
            if (pendingCount > 0) {
                return sendError(res, `Impossible de désactiver cet entrepôt car ${pendingCount} transfert(s) inter-dépôts sont en attente ou en cours. Veuillez d'abord les confirmer ou les annuler.`, 400);
            }
        }
    ...
    `, [updatedName, updatedCode, updatedLocation, updatedPhone, updatedActive, id]);
        ...
        await logAudit(req.user, auditAction, 'WAREHOUSE', id, auditDesc, id, JSON.stringify({ name: current.name, code: current.code, active: current.active }), JSON.stringify({ name: updatedName, code: updatedCode, active: updatedActive }));
