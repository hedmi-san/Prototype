...
router.get('/', authenticate, async (req, res) => {
    ...
    `;
      ...
      SELECT s.id, s.invoice_number, s.warehouse_id, w.name as warehouse_name, w.code as warehouse_code,
             ...
             cl.is_default as client_is_default,
    ...
    `;
      ...
      `, [saleIds]);
        ...
        const items = salesRows.map((s) => ({
            ...
            clientIsDefault: Boolean(s.client_is_default || s.client_code === 'CLT-COMPTOIR'),
...
router.get('/fulfillment-lines/pending', authenticate, async (req, res) => {
    try {
...
router.get('/fulfillment-lines/pending-count', authenticate, async (req, res) => {
    try {
        await checkAndExpireReservations();
        const requestedWarehouseId = req.query.warehouseId ? Number(req.query.warehouseId) : undefined;
        const warehouseId = enforceWarehouseScope(req.user, requestedWarehouseId);
...
router.post('/fulfillment-lines/:id/fulfill', authenticate, async (req, res) => {
    try {
...
router.post('/fulfillment-lines/:id/cancel', authenticate, async (req, res) => {
    try {
        const lineId = Number(req.params.id);
        const result = await runTransaction(async (client) => {
            const lineRes = await client.query(`
      ...
      `, [lineId]);
            const line = lineRes.rows[0];
            if (!line)
                throw new Error(`Ligne introuvable avec l'id ${lineId}`);
            ...
            // Release reservation
            const resQuery = await client.query("SELECT id FROM stock_reservations WHERE fulfillment_line_id = $1 AND status = 'ACTIVE'", [lineId]);
            if (resQuery.rows[0]) {
      ...
      `, [line.sale_id]);
            const activeFulfilled = linesCheck.rows.filter((l) => l.fulfillment_status === 'FULFILLED');
            const activePending = linesCheck.rows.filter((l) => l.fulfillment_status === 'PENDING_PICKUP');
...
router.put('/fulfillment-lines/:id', authenticate, async (req, res) => {
    try {
        const lineId = Number(req.params.id);
...
router.get('/:id', authenticate, async (req, res) => {
    ...
    `, [s.id]);
      ...
      ORDER BY fl.id ASC
    `, [s.id]);
        const fulfillmentLines = flRes.rows.map((fl) => ({
            id: fl.id,
            saleId: fl.sale_id,
...
router.post('/', authenticate, async (req, res) => {
    try {
        const { warehouseId, employeeId, clientId, customerName, customerPhone, saleDate, items, paymentCondition, // 'FULL_CASH' | 'CREDIT' | 'PARTIAL_DOWNPAYMENT'
        downpaymentAmount, paymentMethod, useAdvanceCredit, fulfillmentAllocations, // Optional FulfillmentAllocationInput[]
         } = req.body;
        ...
        const hasInterWarehouse = hasAllocations && fulfillmentAllocations.some((a) => Number(a.fulfillmentWarehouseId) !== Number(targetWarehouseId));
        ...
        const sale = await runTransaction(async (client) => {
            ...
            if (hasAllocations) {
                ...
                for (const alloc of fulfillmentAllocations) {
                    ...
                    if (!product)
                        throw new Error(`Product not found with id ${alloc.productId}`);
                    ...
                    if (Number(alloc.fulfillmentWarehouseId) === Number(targetWarehouseId)) {
                        ...
                        const stockRes = await client.query('SELECT * FROM stock WHERE warehouse_id = $1 AND product_id = $2 FOR UPDATE', [targetWarehouseId, alloc.productId]);
                        ...
                        await client.query('UPDATE stock SET physical_quantity = physical_quantity - $1, updated_at = NOW() WHERE id = $2', [alloc.quantity, stock.id]);
                        await client.query(`
              INSERT INTO stock_movements (warehouse_id, product_id, movement_type, quantity_change, reference, notes)
              VALUES ($1, $2, 'SALE', $3, $4, $5)
            `, [targetWarehouseId, alloc.productId, -Number(alloc.quantity), invoiceNumber, `Sale to ${customerName || clientRecord?.name || 'Client'}`]);
                ...
                for (const item of items) {
                    ...
                    const stockRes = await client.query('SELECT * FROM stock WHERE warehouse_id = $1 AND product_id = $2 FOR UPDATE', [targetWarehouseId, item.productId]);
                    const stock = stockRes.rows[0];
                    const physQty = stock ? Number(stock.physical_quantity) : 0;
                    const resQty = stock ? Number(stock.reserved_quantity) : 0;
                    const available = physQty - resQty;
      ...
      `, [
        ...
        await logAudit(req.user, 'SALE_CREATED', 'SALE', sale.id, `Création vente ${sale.invoiceNumber} (Total: ${sale.totalAmount} DZD, Payé: ${sale.paidAmount} DZD, Statut: ${sale.paymentStatus})`, targetWarehouseId);
        if (sale.isOptedOut) {
            await logAudit(req.user, 'SALE_CREDIT_OPT_OUT', 'SALE', sale.id, `L'avoir disponible de ${sale.availableAdvance.toFixed(2)} DZD n'a pas été appliqué sur la facture ${sale.invoiceNumber} pour le client ${sale.clientName} (choix caissier/client)`, targetWarehouseId);
        }
...
router.delete('/:id', authenticate, requireRole('ADMIN', 'SUPER_MANAGER'), async (req, res) => {
    try {
        const id = Number(req.params.id);
...
router.post('/:id/cancel', authenticate, requireRole('ADMIN', 'SUPER_MANAGER', 'MANAGER'), async (req, res) => {
    try {
        const id = Number(req.params.id);
        const currentRes = await query('SELECT * FROM sales WHERE id = $1', [id]);
        const currentSale = currentRes.rows[0];
        ...
        if (currentSale.has_inter_warehouse_fulfillment) {
            ...
            if (fulfillmentLines.length > 0) {
                const fulfilledLines = fulfillmentLines.filter((l) => l.fulfillment_status === 'FULFILLED');
                const pendingLines = fulfillmentLines.filter((l) => l.fulfillment_status === 'PENDING_PICKUP');
                ...
                if (fulfilledLines.length === 0 && pendingLines.length > 0) {
                    const result = await runTransaction(async (client) => {
                        for (const line of pendingLines) {
                            const resQuery = await client.query("SELECT id FROM stock_reservations WHERE fulfillment_line_id = $1 AND status = 'ACTIVE'", [line.id]);
                            if (resQuery.rows[0]) {
                                await releaseStockReservation(client, resQuery.rows[0].id, 'CANCELLED');
                            }
                            await client.query(`
