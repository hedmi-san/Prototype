...
function seedData() {
    const roleCount = database_js_1.db.prepare('SELECT COUNT(*) as count FROM roles').get().count;
    if (roleCount > 0) {
        return;
    }
    (0, database_js_1.runTransaction)(() => {
        // 1. Roles
        const insertRole = database_js_1.db.prepare('INSERT INTO roles (id, name, description) VALUES (?, ?, ?)');
        ...
        // 2. Warehouses
        const insertWarehouse = database_js_1.db.prepare('INSERT INTO warehouses (id, name, code, location, contact_number, active) VALUES (?, ?, ?, ?, ?, 1)');
        ...
        const adminPass = bcryptjs_1.default.hashSync('AdminPass123!', salt);
        const managerPass = bcryptjs_1.default.hashSync('ManagerPass123!', salt);
        const superPass = bcryptjs_1.default.hashSync('SuperPass123!', salt);
        const accountantPass = bcryptjs_1.default.hashSync('AccountantPass123!', salt);
        const insertUser = database_js_1.db.prepare('INSERT INTO users (username, password_hash, full_name, role_id, warehouse_id, active) VALUES (?, ?, ?, ?, ?, 1)');
        ...
        // 4. Products
        const insertProduct = database_js_1.db.prepare(`
    ...
    `);
        ...
        const insertStock = database_js_1.db.prepare('INSERT INTO stock (warehouse_id, product_id, physical_quantity, reserved_quantity) VALUES (?, ?, ?, 0)');
        const insertMovement = database_js_1.db.prepare(`
    ...
    `);
        ...
        // 6. Demo Employees
        const insertEmp = database_js_1.db.prepare('INSERT INTO employees (id, warehouse_id, full_name, national_id, phone, position, base_salary, active) VALUES (?, ?, ?, ?, ?, ?, ?, 1)');
        ...
        // 7. Demo Expenses
        const insertExp = database_js_1.db.prepare('INSERT INTO expenses (warehouse_id, category, amount, description, expense_date) VALUES (?, ?, ?, ?, ?)');
        ...
        const insertSalary = database_js_1.db.prepare('INSERT INTO salaries (employee_id, warehouse_id, period, base_salary, bonus1, bonus2, total_amount, payment_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?)');
        ...
        // 9. Initial Audit Log
        const insertAudit = database_js_1.db.prepare(`
