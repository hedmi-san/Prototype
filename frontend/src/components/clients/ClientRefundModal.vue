...
import type { Client, ClientDetail, ClientRefund, CounterCreditNote } from '../../types';
...
interface Props {
  ...
  creditNote?: CounterCreditNote | null;
}
...
const recipientIdCard = ref('');
const notes = ref('');
const submitting = ref(false);
const errorMessage = ref('');

const currentBalance = computed(() => Number(props.client?.currentBalance || 0));
const isWalkin = computed(() => Boolean(props.client?.isDefault));

// Client has an advance if balance < 0 (nominative) OR if targeting an active credit note
const hasCreditNote = computed(() => Boolean(props.creditNote && Number(props.creditNote.remainingAmount) > 0));
const hasAdvance = computed(() => {
  if (isWalkin.value) {
    return hasCreditNote.value;
  }
  return hasCreditNote.value || currentBalance.value < 0;
});

const availableAdvance = computed(() => {
  if (hasCreditNote.value && props.creditNote) {
    return Number(props.creditNote.remainingAmount);
  }
  return currentBalance.value < 0 ? Math.abs(currentBalance.value) : 0;
});
...
watch(
  ...
  (isOpen) => {
    if (isOpen) {
      ...
      recipientIdCard.value = '';
      notes.value = '';
      selectedWarehouseId.value = props.creditNote?.warehouseId || authStore.activeWarehouseId || warehouseStore.warehouses[0]?.id || 1;
...
async function handleSubmit() {
  ...
  if (!isValid.value) {
    if (isAmountExceeded.value) {
      errorMessage.value = `Le montant ne peut pas dépasser l'avoir disponible (${formatCurrency(availableAdvance.value)}).`;
    } else if ((amount.value || 0) <= 0) {
      errorMessage.value = 'Veuillez saisir un montant supérieur à 0 DZD.';
    } else if (isWalkin.value && (!recipientName.value.trim() || !recipientPhone.value.trim())) {
      errorMessage.value = 'Le nom complet et le téléphone du bénéficiaire sont obligatoires pour un client passager.';
    }
    return;
  }
  ...
  try {
    const refund = await clientService.refundClientAdvance(props.client.id, {
      ...
      creditNoteId: props.creditNote?.id || null,
      recipientName: recipientName.value.trim() || undefined,
      recipientPhone: recipientPhone.value.trim() || undefined,
      recipientIdCard: recipientIdCard.value.trim() || undefined,
  ...
  >
    <div class="refund-modal-body">
      ...
      <div v-if="!hasAdvance" class="warning-banner">
        ...
        <div v-if="isWalkin">
          <strong>Aucun avoir comptoir remboursable</strong>
          <p>Pour un client passager, tout remboursement en espèces doit être rattaché à un avoir comptoir valide en attente. Veuillez sélectionner un avoir actif depuis l'onglet « Avoirs Comptoir ».</p>
        </div>
        <div v-else>
      ...
      <div v-else class="client-advance-banner">
        <div class="client-banner-header">
          <div class="client-info">
            ...
            <span v-if="props.creditNote" class="credit-note-tag font-mono">
              Avoir n° {{ props.creditNote.creditNoteNumber }}
              <template v-if="props.creditNote.saleInvoiceNumber"> (Vente {{ props.creditNote.saleInvoiceNumber }})</template>
            </span>
          </div>
          <div class="advance-pill">
            <span class="advance-pill-label">{{ props.creditNote ? "Reste de l'Avoir" : 'Avance Disponible' }}</span>
      ...
      <form v-if="hasAdvance" @submit.prevent="handleSubmit" class="refund-form">
        ...
        <div class="form-group">
          ...
          <p v-if="isAmountExceeded" class="input-error-msg">
            Le montant dépasse le plafond d'avoir disponible de {{ formatCurrency(availableAdvance) }}.
          </p>
        </div>

        <!-- Beneficiary section for walk-in or credit note -->
        <div v-if="isWalkin || hasCreditNote" class="beneficiary-card">
          <div class="beneficiary-header">
            <div class="beneficiary-title">
              ...
              <span>Bénéficiaire Réceptionnaire (Bon de Décharge)</span>
            </div>
            <span v-if="isWalkin" class="required-pill">Obligatoire</span>
          </div>

          <div class="form-grid-2">
            <div class="form-group">
              <label class="form-label" for="recipient-name">
                Nom & Prénom du Bénéficiaire {{ isWalkin ? '*' : '' }}
              </label>
              <input
                id="recipient-name"
          ...
          <div class="form-group" style="margin-top: 0.5rem;">
            <label class="form-label" for="recipient-id">
              N° Pièce d'Identité / CNI (Optionnel)
            </label>
            <input
              id="recipient-id"
              v-model="recipientIdCard"
              type="text"
              class="form-input"
        ...
        <div class="form-group">
          ...
          <textarea
            ...
            placeholder="Ex: Restitution d'avoir comptoir suite annulation..."
          ></textarea>
        </div>
