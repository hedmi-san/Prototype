...
import type { ClientDetail, StatementOfAccount, Sale, ClientPayment, ClientRefund, CounterCreditNote, CreditNoteStatus } from '../../types';
...
const activeTab = ref<'statement' | 'invoices' | 'payments' | 'creditNotes'>('statement');
...
// Credit Notes State & Pagination
const creditNotes = ref<CounterCreditNote[]>([]);
const creditNotesLoading = ref(false);
const creditNotesPage = ref(1);
const creditNotesLimit = ref(25);
const creditNotesTotal = ref(0);
const creditNotesTotalPages = ref(1);
const creditNotesStatusFilter = ref<string>('all');
const activeCreditNotesCount = ref(0);
const selectedCreditNoteForRefund = ref<CounterCreditNote | null>(null);
const selectedCreditNoteForReceipt = ref<CounterCreditNote | null>(null);
const showCreditNoteReceiptModal = ref(false);
...
// Reactivate & Forfeit Dialogs
const showReactivateDialog = ref(false);
const reactivatingCreditNote = ref<CounterCreditNote | null>(null);
const reactivating = ref(false);

const showForfeitDialog = ref(false);
const forfeitingCreditNote = ref<CounterCreditNote | null>(null);
...
watch(activeTab, async (newTab) => {
  if (newTab === 'invoices' && invoices.value.length === 0) {
    ...
    await loadPayments();
  } else if (newTab === 'creditNotes' && creditNotes.value.length === 0) {
...
async function loadClientData() {
  ...
  try {
    ...
    if (clientRes.isDefault || activeTab.value === 'creditNotes') {
      await loadCreditNotes();
    }
  } catch (err) {
...
function handleOpenRefundModal() {
  selectedCreditNoteForRefund.value = null;
...
function handleRefundSaved(refund: ClientRefund) {
  ...
  loadClientData();
  if (client.value?.isDefault || activeTab.value === 'creditNotes') {
    loadCreditNotes();
  }
}
...
async function loadCreditNotes() {
  ...
  try {
    const res = await clientService.getCreditNotes({
      clientId: clientId.value,
      status: creditNotesStatusFilter.value !== 'all' ? creditNotesStatusFilter.value : undefined,
      page: creditNotesPage.value,
      limit: creditNotesLimit.value,
    });
    creditNotes.value = res.items;
    creditNotesTotal.value = res.pagination.total;
    creditNotesTotalPages.value = res.pagination.totalPages;
    activeCreditNotesCount.value = res.items.filter(
      (c) => c.status === 'PENDING' || c.status === 'PARTIALLY_REFUNDED'
    ).length;
  } catch (err) {
...
function handleRefundCreditNote(cn: CounterCreditNote) {
  selectedCreditNoteForRefund.value = cn;
  showRefundModal.value = true;
}

function handlePrintCreditNote(cn: CounterCreditNote) {
  selectedCreditNoteForReceipt.value = cn;
  showCreditNoteReceiptModal.value = true;
}

function promptReactivateCreditNote(cn: CounterCreditNote) {
  reactivatingCreditNote.value = cn;
  showReactivateDialog.value = true;
}

async function handleConfirmReactivate() {
  if (!reactivatingCreditNote.value) return;
  reactivating.value = true;
  try {
    await clientService.reactivateCreditNote(
      reactivatingCreditNote.value.id,
      'Réactivation administrative autorisée par la direction'
    );
    showReactivateDialog.value = false;
    await Promise.all([loadCreditNotes(), loadClientData()]);
  } catch (err) {
    console.error('Failed to reactivate credit note', err);
  } finally {
    reactivating.value = false;
  }
}

function promptForfeitCreditNote(cn: CounterCreditNote) {
  forfeitingCreditNote.value = cn;
  showForfeitDialog.value = true;
}

async function handleConfirmForfeit() {
  if (!forfeitingCreditNote.value) return;
  forfeiting.value = true;
  try {
    await clientService.forfeitCreditNote(
      forfeitingCreditNote.value.id,
      "Constatation forclusion et extinction de l'avoir non réclamé (Compte 758)"
    );
    showForfeitDialog.value = false;
    await Promise.all([loadCreditNotes(), loadClientData()]);
  } catch (err) {
    console.error('Failed to forfeit credit note', err);
  } finally {
    forfeiting.value = false;
  }
}

function getCreditNoteExpiryInfo(expiryDate: string, status: CreditNoteStatus) {
  if (status === 'FULLY_REFUNDED') return { text: 'Soldé', isExpired: false, isExpiringSoon: false };
  if (status === 'FORFEITED') return { text: 'Forclos (Éteint)', isExpired: true, isExpiringSoon: false };
  const exp = new Date(expiryDate).getTime();
  const now = Date.now();
  const diffDays = Math.ceil((exp - now) / (1000 * 60 * 60 * 24));
  if (diffDays < 0 || status === 'EXPIRED') {
    return { text: `Expiré (${Math.abs(diffDays)} j)`, isExpired: true, isExpiringSoon: false };
  }
  if (diffDays <= 7) {
    return { text: `Expire dans ${diffDays} j !`, isExpired: false, isExpiringSoon: true };
  }
  return { text: `Valable (${diffDays} j restants)`, isExpired: false, isExpiringSoon: false };
}

function getCreditNoteStatusLabel(status: CreditNoteStatus) {
  switch (status) {
    case 'PENDING': return 'En Attente';
    case 'PARTIALLY_REFUNDED': return 'Partiel';
    case 'FULLY_REFUNDED': return 'Soldé';
    case 'EXPIRED': return 'Expiré';
    case 'FORFEITED': return 'Forclos (Éteint)';
    default: return status;
  }
}

function getCreditNoteStatusVariant(status: CreditNoteStatus): 'warning' | 'info' | 'success' | 'danger' | 'neutral' {
  switch (status) {
    case 'PENDING': return 'warning';
    case 'PARTIALLY_REFUNDED': return 'info';
    case 'FULLY_REFUNDED': return 'success';
    case 'EXPIRED': return 'danger';
    case 'FORFEITED': return 'neutral';
...
<template>
  <div class="client-profile-view">
    ...
    <div class="profile-nav no-print">
      ...
      <div class="profile-actions">
        ...
        <AppButton
          ...
          variant="secondary"
          size="sm"
          title="Consulter et rembourser les avoirs comptoir"
        ...
        >
          ...
          Avoirs Comptoir
          <span v-if="activeCreditNotesCount > 0" class="badge-mini-count">{{ activeCreditNotesCount }}</span>
        </AppButton>
        <AppButton
          v-if="!client?.isDefault"
          variant="secondary"
    ...
    <div class="tabs-nav no-print">
      ...
      <button
        :class="['tab-btn', { active: activeTab === 'creditNotes' }]"
        @click="activeTab = 'creditNotes'"
      >
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" />
        ...
        Avoirs Comptoir & Restitutions
        <span v-if="activeCreditNotesCount > 0" class="tab-badge-pill">{{ activeCreditNotesCount }}</span>
      </button>
    </div>
    ...
    <!-- TAB 4: AVOIRS COMPTOIR & RESTITUTIONS (CREDIT NOTES) -->
    <div v-show="activeTab === 'creditNotes'" class="tab-pane">
      <div class="tab-table-card">
        <!-- Filter & Header Ribbon -->
        <div class="credit-notes-filter-bar no-print">
          <div class="filter-left">
            <label class="filter-label">Statut :</label>
            <select v-model="creditNotesStatusFilter" class="filter-select" @change="loadCreditNotes">
              <option value="all">Tous les avoirs</option>
              <option value="PENDING">En attente (Actifs)</option>
              <option value="PARTIALLY_REFUNDED">Partiellement remboursés</option>
              <option value="FULLY_REFUNDED">Soldés (Remboursés)</option>
              <option value="EXPIRED">Expirés (+90 jours)</option>
              <option value="FORFEITED">Forclos (Extinction Compte 758)</option>
            </select>
          </div>
          <div class="filter-right">
            <AppButton variant="secondary" size="sm" :loading="creditNotesLoading" @click="loadCreditNotes">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        ...
        <div v-if="creditNotesLoading" class="pane-loading">
          <div class="spinner"></div>
          <span>Chargement des avoirs comptoir...</span>
        </div>
        <div v-else-if="creditNotes.length === 0" class="pane-empty">
          <p>Aucun avoir comptoir enregistré pour ce client.</p>
        </div>
        <div v-else class="table-container">
          <table class="data-table">
            <thead>
              <tr>
                ...
                <th>Vente Liée</th>
                <th>Émis le</th>
                <th>Date Limite / Statut Validité</th>
                <th>Dépôt Émetteur</th>
                <th class="text-right">Montant Initial</th>
                <th class="text-right">Reste Remboursable</th>
                <th>Statut</th>
            ...
            <tbody>
              <tr v-for="cn in creditNotes" :key="cn.id">
                <td>
                  <strong class="font-mono font-bold text-primary">{{ cn.creditNoteNumber }}</strong>
                </td>
                <td>
                  <span v-if="cn.saleInvoiceNumber" class="font-mono font-bold">{{ cn.saleInvoiceNumber }}</span>
                  <span v-else class="text-muted text-xs">-</span>
                </td>
                <td>{{ formatDate(cn.issueDate) }}</td>
                <td>
                  <div class="expiry-cell">
                    <span>{{ formatDate(cn.expiryDate) }}</span>
                    <span
                      :class="['expiry-pill', {
                        expired: getCreditNoteExpiryInfo(cn.expiryDate, cn.status).isExpired,
                        urgent: getCreditNoteExpiryInfo(cn.expiryDate, cn.status).isExpiringSoon
                      }]"
                    >
                      {{ getCreditNoteExpiryInfo(cn.expiryDate, cn.status).text }}
                    </span>
                  </div>
                </td>
                <td>{{ cn.warehouseName || cn.warehouseCode || 'Comptoir' }}</td>
                <td class="text-right font-mono">{{ formatCurrency(cn.totalAmount) }}</td>
                <td class="text-right font-mono font-bold" :class="cn.remainingAmount > 0 ? 'text-danger' : 'text-muted'">
                  {{ formatCurrency(cn.remainingAmount) }}
                </td>
                <td>
                  <AppBadge :variant="getCreditNoteStatusVariant(cn.status)">
                    {{ getCreditNoteStatusLabel(cn.status) }}
                  </AppBadge>
                </td>
                <td class="text-right">
                  <div class="cn-action-buttons">
                    <!-- Refund Action (if active) -->
                    <button
                      v-if="cn.status === 'PENDING' || cn.status === 'PARTIALLY_REFUNDED'"
                      class="cn-btn refund"
                      title="Décaisser et rembourser cet avoir en espèces"
                      @click="handleRefundCreditNote(cn)"
                    >
                    ...
                    <!-- Reactivate Action (Admin / Super Manager if EXPIRED) -->
                    <button
                      v-if="cn.status === 'EXPIRED' && (authStore.isAdmin || authStore.isSuperManager)"
                      class="cn-btn reactivate"
                      title="Réactiver l'avoir expiré"
                      @click="promptReactivateCreditNote(cn)"
                    >
                      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <polyline points="23 4 23 10 17 10" />
                        <path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10" />
                    ...
                    <!-- Forfeit Action (Accountant / Admin if EXPIRED) -->
                    <button
                      v-if="cn.status === 'EXPIRED' && (authStore.isAdmin || authStore.isAccountant)"
                      class="cn-btn forfeit"
                      title="Constater la forclusion et solder en compte 758"
                      @click="promptForfeitCreditNote(cn)"
                    >
                      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    ...
                    <!-- Printable Receipt -->
                    <button
                      class="cn-btn receipt"
                      title="Imprimer / Afficher le reçu d'avoir"
                      @click="handlePrintCreditNote(cn)"
                    >
                      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <polyline points="6 9 6 2 18 2 18 9" />
                        <path d="M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2" />
        ...
        <div v-if="creditNotesTotalPages > 1" class="tab-pagination no-print">
          <AppPagination
            v-model:page="creditNotesPage"
            v-model:limit="creditNotesLimit"
            :total="creditNotesTotal"
            :total-pages="creditNotesTotalPages"
    ...
    <ClientRefundModal
      ...
      :client="client"
      :credit-note="selectedCreditNoteForRefund"
    ...
    <!-- Credit Note Receipt Document Modal -->
    <CreditNoteReceipt
      v-model="showCreditNoteReceiptModal"
      :credit-note="selectedCreditNoteForReceipt"
    />

    <!-- Reactivate Credit Note Dialog -->
    <ConfirmDialog
      v-model="showReactivateDialog"
      title="Réactiver l'Avoir Comptoir Expiré"
      :message="`Êtes-vous sûr de vouloir réactiver l'avoir n° ${reactivatingCreditNote?.creditNoteNumber} ? Son statut repassera en attente, prolongeant sa validité de 30 jours et autorisant son remboursement au guichet.`"
      confirm-text="Confirmer la Réactivation"
      cancel-text="Annuler"
      variant="primary"
      :loading="reactivating"
      @confirm="handleConfirmReactivate"
    />

    <!-- Forfeit Credit Note Dialog -->
    <ConfirmDialog
      v-model="showForfeitDialog"
      title="Constater la Forclusion de l'Avoir (Comptabilité)"
      :message="`Attention : vous vous apprêtez à constater la forclusion définitive de l'avoir n° ${forfeitingCreditNote?.creditNoteNumber} pour un montant de ${formatCurrency(forfeitingCreditNote?.remainingAmount || 0)}. Cette opération va générer une écriture financière DÉBIT sur le compte passager et solder la dette en CREDIT Compte 758 (Produit Exceptionnel). Confirmer ?`"
      confirm-text="Constater la Forclusion & Solder"
      cancel-text="Annuler"
      variant="danger"
      :loading="forfeiting"
      @confirm="handleConfirmForfeit"
    />
  </div>
</template>
...
.credit-notes-filter-bar .filter-left {
  display: flex;
  align-items: center;
  gap: 10px;
}

.credit-notes-filter-bar .filter-label {
  font-size: 0.85rem;
  font-weight: 600;
